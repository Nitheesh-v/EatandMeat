...
export const Earnings = () => {
  ...
  const cards = [
    { title: "Today", amount: `₹${earnings?.today?.amount || 0}`, orders: earnings?.today?.orders || 0, icon: IndianRupee, accent: "#9333ea", bg: "rgba(147,51,234,0.08)" },
    { title: "This Week", amount: `₹${earnings?.week?.amount || 0}`, orders: earnings?.week?.orders || 0, icon: Wallet, accent: "#ec4899", bg: "rgba(236,72,153,0.08)" },
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-extrabold" style={{ color: "#1e293b" }}>Earnings</h1>
        <p className="text-sm mt-1" style={{ color: "#94a3b8" }}>Track your delivery income</p>
      </div>
          ...
          >
            ...
            <div className="p-5">
              <div className="flex items-center justify-between mb-3">
                <p className="text-xs font-semibold" style={{ color: "#94a3b8" }}>{card.title}</p>
              ...
              <h2 className="text-2xl font-extrabold" style={{ color: "#1e293b" }}>{card.amount}</h2>
              <p className="text-[11px] mt-1 flex items-center gap-1" style={{ color: "#94a3b8" }}>
      ...
      {/* History */}
      <div className="rounded-2xl overflow-hidden" style={{ background: "#fff", border: "1px solid rgba(147,51,234,0.08)" }}>
        ...
        <div className="p-5" style={{ borderBottom: "1px solid rgba(0,0,0,0.05)" }}>
          <h2 className="font-bold text-lg" style={{ color: "#1e293b" }}>Delivery History</h2>
        </div>
        {recentOrders.length === 0 ? (
          <p className="text-center py-6" style={{ color: "#94a3b8" }}>No deliveries yet</p>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr style={{ background: "rgba(147,51,234,0.02)" }}>
                  {["Order", "Date", "Items", "Amount", "Status"].map((h) => (
                    <th key={h} className="p-3 text-left text-[11px] font-bold uppercase tracking-wider" style={{ color: "#94a3b8" }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {recentOrders.map((order) => (
                  <tr key={order._id} style={{ borderTop: "1px solid rgba(0,0,0,0.04)" }}>
                    <td className="p-3">
                      <span className="font-bold text-sm" style={{ color: "#9333ea" }}>#{order.orderNumber}</span>
                    </td>
                    <td className="p-3 text-sm" style={{ color: "#64748b" }}>
                      {new Date(order.deliveredAt).toLocaleDateString("en-IN")}
                    </td>
                    <td className="p-3 text-sm" style={{ color: "#64748b" }}>{order.items?.length || 0}</td>
                    <td className="p-3 font-bold text-sm" style={{ color: "#1e293b" }}>₹{order.totalAmount}</td>
                    <td className="p-3">
